...
function GMsgBox(tit,msg,opt,w,h,th,z,idp,op,cls,closeImg){//Opt [{val:,evt:,handler:}]
	var g=this;g.con=randomStr(4)+idp||'';g.cls=cls||0;g.f=1;g.con=ce('div',g.con,gt('body')[0]);g.con.className='gmsgCon'+(g.cls||'');g.tit=tit;g.msg=msg;g.opt=((!opt)?[]:opt);g.z=z||998;g.idp=idp||"a";g.bop=0.8;g.w=w||350;g.h=h||200;g.th=th||25;g.op=op||0.9;g.closeImg=closeImg||0;
...
GMsgBox.prototype.box=function(){
	...
	//g1mbb
	t1.id='g1mbb'+g.idp;t1.className='g1mbb'+(g.cls||'');g.g1mbb=t1;t1.style.backgroundColor="rgb(41,41,41)";t1.style.width=Math.min(d.clientWidth,d.scrollWidth)+"px";t1.style.height=(Math.max(d.scrollHeight,d.clientHeight))+"px";t1.style.position="absolute";
	t1.style.zIndex=g.z;t1.style.opacity=g.bop;t1.style.filter="alpha(opacity="+g.bop*100+")";t1.style.top=0;t1.style.left=0;ac(c,t1);t1=ce('div');
	//g1mbb end
	t1.id="g2mbb"+g.idp;t1.className='g2mbb'+(g.cls||'');g.g2mbb=t1;t1.style.width=g.w+"px";t1.style.height=g.h+"px";t1.style.position="absolute";t1.style.zIndex=g.z;
	// t1.style.left=(d.clientWidth/2-g.w/2)+"px";
	...
	if(g.opt.length){
		t2=ce('div');t2.id='g2tba'+g.idp;t2.className='g2tba'+(g.cls||'');if(!g.cls){t2.style.textAlign="center";t2.style.margin="0 auto";t2.style.padding="0 15px 15px 15px";t2.style.position="absolute";t2.style.bottom="0";}t2.style.width=(g.w)+"px";ac(t1,t2);
...
GMsgBox.prototype.unbox=function(e){
	...
	//added vs
	removeClass(gt('body')[0],'modal-open');
};
...
GMsgBox.prototype.pos=function(){
	return;//using postion absolute margin auto instead to center
	var g=this,s=(document.documentElement&&document.documentElement.scrollTop)||document.body.scrollTop;s=s||0;g.g2mbb.style.top=(s+(window.innerHeight||document.documentElement.offsetHeight)/2-g.h/2)+"px";
};
...
window.onhashchange=function(e){
	if(gv.allowHashUpdation){showSect(getHash(),false);}
	gv.allowHashUpdation=true;
};
function getHash(){return window.location.hash.split('?')[0].substring(1);}
function setHash(str){if(getHash()==str)return;gv.allowHashUpdation=false;window.location.hash=str;}
...
function removeClass(el,cls){if(hasClass(el,cls)){
		var reg=new RegExp('(\\s|^)'+cls+'(\\s|$)');
		el.className=el.className.replace(reg,' ');
	}
}
function indexOfData(a,key,val){
	for(var i=0,lx=a.length;i<lx;i++){if(a[i][key]==val)return i;}
		return -1;
}
function getValFromURL(key) { //querystring  -key value pairs
...
function scrollSmooth(ey,dur,cb){
	if(!dur)dur=500;
	var by=document.documentElement.scrollTop||document.body.scrollTop;
	var cy=ey-by,lxi=dur*60/1000,y;
	var i=0,requestID,step=function(){
		y=Math.easeInOutQuad(i,by,cy,lxi);
		window.scrollTo(0,y);
		if (i<lxi){
			i++;requestID = requestAnimationFrame(step);
		}else{
...
Math.easeOutElastic=function (t, b, c, d) {
	var s=2.5;var p=0;var a=c;
	if (t==0) return b;  if ((t/=d)==1) return b+c;  if (!p) p=d*.3;
	if (a < Math.abs(c)) { a=c; var s=p/4; }
	else var s = p/(2*Math.PI) * Math.asin (c/a);
	return a*Math.pow(2,-10*t) * Math.sin( (t*d-s)*(2*Math.PI)/p ) + c + b;
}
Math.easeOutBack=function (t, b, c, d) {
	var s = 1.70158;
